...
export async function POST(_request: NextRequest) {
  try {
    ...
    if (!data.product_id || !data.warehouse_id || data.quantity === undefined || data.min_quantity === undefined) {
      return NextResponse.json({
        error: 'Product ID, warehouse ID, quantity, and minimum quantity are required' }, { status: 400 });
    }
    ...
    if (existingInventory) {
      return NextResponse.json({
        error: 'Inventory already exists for this product in this warehouse' }, { status: 409 });
    }
