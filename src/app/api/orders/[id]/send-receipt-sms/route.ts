...
export async function POST(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const message = `Payoff Solar receipt for order #${order.id.substring(0,8)}: ${receiptUrl}`;
    ...
    try {
      ...
      const publishRes = await sns.send(new PublishCommand({ PhoneNumber: e164, Message: message, MessageAttributes: msgAttrs }));
    ...
    try {
      ...
      console.error('Error tracking receipt SMS:', trackingError);
