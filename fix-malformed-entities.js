...
// Function to fix malformed entities
function fixFile(filePath) {
  let content = fs.readFileSync(filePath, 'utf8');
  let modified = false;

  // Fix malformed entities that break JSX parsing
  const malformedEntityFixes = [
    // Fix incomplete entities at the end of expressions
    [/&apos;(?=\s*[:\}\)\]\,\;])/g, "'"],
    [/&quot;(?=\s*[:\}\)\]\,\;])/g, '"'],
    
    // Fix entities in template literals
  ...
  const standaloneEntityFixes = [
    // Fix entities that are clearly in the wrong place
  ...
  standaloneEntityFixes.forEach(([pattern, replacement]) => {
    if (pattern.test(content)) {
      content = content.replace(pattern, replacement);
      modified = true;
    }
  ...
  // Ensure proper JSX text entities (only in actual text content)
  const jsxTextFixes = [
